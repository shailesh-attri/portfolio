  ...
  return (
    <div className="overflow-scroll h-[800px] px-4 p-4">
      <div className="flex items-center justify-start gap-2">
        <a href='#skills' className="bg-gray-800 text-brand-fill py-1 px-2 font-bold rounded font-Lora">Skills</a>
        <a href='#education' className="bg-gray-800 text-brand-fill py-1 px-2 font-bold rounded font-Lora">Education</a>
      </div>
      <h1 className="text-[2rem] font-bold font-Lora">Hello friends 👋!!</h1>
      <p className="mb-4 text-gray-300 font-LoraLight">
      ...
      <h1 className="font-bold text-[2rem] text-gray-300 font-Lora" id='skills'>Skills</h1>
      <div className="flex items-center justify-center gap-2 flex-wrap mb-8">
        {skills && skills?.map((skill, i) => (
          <span key={i} className="font-LoraLight border-2 border-gray-600 px-4 py-2 text-brand-fill rounded-lg">{skill}</span>
        ))}
      </div>
      <h1 className="font-bold text-[2rem] text-gray-300 font-Lora" id='education'>Education</h1>
      <div className="education">
        <h1 className="font-bold text-brand-stroke">Masters in Computer Application</h1>
        <p className="text-[0.80rem] text-gray-300 mb-4 font-LoraLight">Galgotias university • 2022 - 2024</p>

        <h1 className="font-bold text-brand-stroke">Bachelors in Science</h1>
        <p className="text-[0.80rem] text-gray-300 mb-4 font-LoraLight">SWSU university Meerut • 2018 - 2022</p>

        <h1 className="font-bold text-brand-stroke">Senior secondary School - 12th Grade </h1>
        <p className="text-[0.80rem] text-gray-300 mb-4 font-LoraLight">Pragyan public school • 2018</p>

        <h1 className="font-bold text-brand-stroke">High School - 10th Grade</h1>
        <p className="text-[0.80rem] text-gray-300 mb-4 font-LoraLight">Babuji convent school • 2016</p>
